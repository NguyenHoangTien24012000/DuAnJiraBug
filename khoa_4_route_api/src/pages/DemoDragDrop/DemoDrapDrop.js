                ...
                <div className="bg-dark p-5 col-4">
                    {taskList.map((item,index)=>{
                       ...
                       onDragStart={(e) =>{handleDragStart(e,item,index)}}
                        onDragEnter = {(e) =>{hangdDragEnter(e,item,index)}}
                        onDragEnd = {(e) =>{hangdleDragEnd(e)}}
                       key={index} className={`bg-success text-white m-3 p-3 ${cssDragTag}`} >{item.taskName}</div>
                    })}
                </div>
                <div className="col-2">
